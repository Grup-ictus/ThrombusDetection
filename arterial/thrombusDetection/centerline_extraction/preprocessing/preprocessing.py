...
def preprocessing(master_volume_node):
    ...
    effect.setParameter("GaussianStandardDeviationMm", 0.6)
    effect.self().onApply()

    # segment_editor_widget.setActiveEffectByName("Smoothing")
    # effect = segment_editor_widget.activeEffect()
    # effect.setParameter("SmoothingMethod", "JOINT_TAUBIN")
    # effect.setParameter("JointTaubinSmoothingFactor", 0.1)
    # effect.self().onApply()
    # segment_editor_widget.setActiveEffectByName("Smoothing")
    # effect = segment_editor_widget.activeEffect()
    # effect.setParameter("SmoothingMethod", "MORPHOLOGICAL_CLOSING")
    # effect.setParameter("KernelSizeMm", 0.01)
    # effect.self().onApply()
    ...
    effect.setParameter("MinimumSize", 2500)
    effect.self().onApply()
